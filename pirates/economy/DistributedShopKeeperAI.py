...
def requestMakeShipSale(self, buying, selling, names):
    ...
    if requiredGold is None:
        # Log console error if the price is not found
        self.notify.error("Price not found for shipId: %s. Setting price to zero." % shipId)
        # Set requiredGold to 0
        requiredGold = 0

    # Verify if the player has enough gold
    if requiredGold > av.getGoldInPocket():
        sendResponse(0)
        return         
    ...
    # Define amount here or pass it as an argument to the function
    amount = 1
